...
class CustomLineCounter:
    def __init__(self, start: Point, end: Point, line_orientation: str = "horizontal"):
        """
        ...
        self.line_orientation = line_orientation.lower()

        if self.line_orientation not in ["horizontal", "vertical"]:
            raise ValueError("Orientation must be 'horizontal' or 'vertical'.")
    def get_texts(self) -> List[str]:
        """
        Get the annotation texts based on the flow direction.

        :param line_counter: CustomLineCounter : The line counter from which the counts will be fetched.
        :return: List[str] : The texts to be annotated on the frame.
        """
        if self.line_orientation == "horizontal":
            return [f"Going up: {self.in_count}", f"Going down: {self.out_count}"]
        else:  # line_orientation is vertical
            return [f"Going to the left: {self.out_count}", f"Going to the right: {self.in_count}"]
...
class CustomLineCounterAnnotator:
    ...
    # return the count of people by the flow direction
    def get_annotate_texts(self, line_counter: CustomLineCounter) -> List[str]:
        """
        Get the annotation texts based on the flow direction.
        ...
        :return: List[str] : The texts to be annotated on the frame.
        """
        if line_counter.line_orientation == "horizontal":
            return [f"Going up: {line_counter.in_count}", f"Going down: {line_counter.out_count}"]
        else:  # line_orientation is vertical
            return [f"Going to the left: {line_counter.out_count}", f"Going to the right: {line_counter.in_count}"]
    ...
    def annotate(self, frame: np.ndarray, line_counter: CustomLineCounter) -> np.ndarray:
        ...
        if(line_counter.line_orientation == "horizontal"):
            in_text = f"Going up: {line_counter.in_count}"
            out_text = f"Going down: {line_counter.out_count}"
        else:
            in_text = f"Going to the right: {line_counter.in_count}"
            out_text = f"Going to the left: {line_counter.out_count}"
        ...
        in_text_y = int(
            ...
            in_text_height * 3 + self.text_offset
        )
        ...
        out_text_y = int(
            ...
            out_text_height + self.text_offset
        )
